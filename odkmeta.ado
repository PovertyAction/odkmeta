...
struct `AttribProps' {
	...
	// Column number
	`RS' col
	// Nonzero if the attribute is defined in the form; 0 if it is.
	`RS' form
	// Nonzero if the attribute has a special purpose within the do-file; 0 if
	// it is used only to attach characteristics.
	`RS' special
...
// `Attrib' and `AttribProps' are separated so that the user can freely change
// props (the elements of `AttribProps') but can modify name only through
// `AttribSet'.
struct `Attrib' {
	`SS' name
	`AttribPropsS' props
...
// A collection of field attributes. Attributes are uniquely identified by their
// names.
...
// Adds an attribute with a specified name and void properties to the
// collection, returning a pointer to the attribute's properties.
